...
export const buildRewardPayload = (data:RewardsFormValues, targetAudienceData: any): CreateRewardBody => {
    ...
    if (data.audience === "MANUAL") {
      const manualAudienceId = targetAudienceData?.data
        ?.find((item:any) => item.category === "MANUAL")
        ?.items?.[0]?.id;
      ...
      const payload: CreateRewardBody = {
            ...
            maxDiscountAmount:
              data.offer_type === "PERCENTAGE"
                ? Number(data.maxDiscountAmount)
                : data.offer_type === "AMOUNT"
                  ? Number(data.amount)
                  : data.offer_type === "CASHBACK"
                    ? Number(data.maxCashbackAmount)
                    : undefined,
